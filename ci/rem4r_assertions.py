...
Table (ID / kind / expectation):
  ...
  A4 MANDATORY  ffuf: partial=none; hosts contain app + www; www alive=true
  A5 MANDATORY  vhosts: >=1 misconfig_suspect=true on base app; ZERO records on base www
  A6 DISCLOSURE dnsr: candidates.perms exact count disclosed; per-host cap respected
  A7 MANDATORY  port-check stage done; scanned-IP count == 0
...
def main() -> int:
    ...
    # A4 ffuf hosts + www alive
    ffuf = load_json(TARGET_DIR / "10_subdomains" / "ffuf" / "data.json") or {}
    ...
    row("A4", "MANDATORY",
        "partial: none" in summary_text and app_present and www.get("alive") is True,
        f"hosts={len(hosts)} app={app_present} www.alive={www.get('alive')} "
        f"partial={'partial: none' in summary_text}")

    # A5 vhost suspect flags
    ...
    row("A5", "MANDATORY", len(app_suspects) >= 1 and len(www_records) == 0,
        f"suspect_on_app={len(app_suspects)} {suspect_names} records_on_www={len(www_records)}")

    # A6 dnsr permutation disclosure
    dnsr = load_json(TARGET_DIR / "20_dns" / "dnsx" / "data.json") or {}
    ...
    row("A6", "DISCLOSURE", True,
        ...
        f"summary_present={dnsr_summary.is_file()}")
    ...
    row("A7", "MANDATORY", port_status == "done" and unique_ips == "0",
    ...
    row("A8", "DISCLOSURE", len(suspect_assets) >= 1,
        ...
        f"www_asset_alive={(www_asset or {}).get('alive')}")
